...
const pivotBuilder = ref<{ spec: () => { rows: string[]; columns: string[]; values: string[]; aggregate: string } }>()
const submitting = ref(false)
const environmentOpen = ref(false), environmentLoaded = ref(false), startupError = ref('')
const automationOpen = ref(false), automationLoaded = ref(false)
...
const operation = ref('append'), keys = ref<string[]>([]), columns = ref<string[]>([]), chartType = ref('column')
const left = ref(''), right = ref(''), templateSheet = ref(''), templateStart = ref(2), mapping = ref<Record<string,string>>({})
...
const operations: Record<string, string> = { calculate: '新增计算列', classify: '条件分级', create_table: '无文件建表', append: '合并文件', join: '按字段关联', clean: '清理文本空格', compare: '对账差异', group: '分组求和', melt: '宽表转长表', pivot: '长表转矩阵 / BOM', template: '填写模板', recalculate: 'Excel 重算副本', report: '数据质量报告', chart: '生成图表', pivot_table: '生成透视表' }
const chartTypes = [{ value: 'column', label: '柱状图' }, { value: 'bar', label: '横条图' }, { value: 'line', label: '折线图' }, { value: 'area', label: '面积图' }, { value: 'pie', label: '饼图' }]
const newRule = computed(() => ['calculate','classify','create_table'].includes(operation.value))
const usesKeys = computed(() => ['join','compare','group','melt','chart'].includes(operation.value))
const usesColumns = computed(() => ['clean','compare','group','melt','template','chart'].includes(operation.value))
const isPivot = computed(() => ['pivot','pivot_table'].includes(operation.value))
const twoFiles = computed(() => ['join','compare','template'].includes(operation.value))
const displayedEvents = computed(() => events.value.filter(e => ['message','user','progress','error','step'].includes(e.kind)))
const liveStreamText = computed(() => {
  const list = events.value
  let start = list.length
  while (start > 0 && list[start - 1].kind === 'stream') start--
  return list.slice(start).map(e => typeof e.data === 'string' ? e.data : (e.data?.text || '')).join('')
})
function stepRows(data: any): string {
  const out = data.rows_out != null ? `${data.rows_out} 行` : ''
  if (Array.isArray(data.rows_in)) return `输入 ${data.rows_in.join(' / ')} 行 → 输出 ${out}`
  if (data.rows_in != null) return `输入 ${data.rows_in} 行 → 输出 ${out}`
  return out ? `输出 ${out}` : ''
}
...
function resetOperation() { if (twoFiles.value && left.value) activeFile.value = left.value; keys.value = []; columns.value = []; mapping.value = {}; chartType.value = 'column' }
async function execute() {
  const current = session.capture()
  await workbook.flush()
  if (!current()) return
  if (!task.value?.files.length && operation.value!=='create_table' && view.value!=='editor') throw new Error('请先添加文件或新建工作簿')
  let inputs: (Selection | WorkbookSelection)[] = operation.value==='create_table' ? [] : view.value==='editor' && !twoFiles.value && !isPivot.value ? [bookInput()] : operation.value === 'append' ? task.value!.files.map(f => selectedInput(f.id)) : [selectedInput(activeFile.value)]
  ...
  if (isPivot.value) {
    if (!activeFile.value) throw new Error('请先添加并选择一个文件')
    const spec = pivotBuilder.value!.spec()
    if (!spec.rows.length) throw new Error('请选择至少一个行字段')
    if (!spec.values.length) throw new Error('请选择至少一个数值字段')
    delete params.keys; delete params.validate; delete params.how
    if (operation.value === 'pivot') Object.assign(params, { rows: spec.rows, columns: spec.columns, values: spec.values, aggregate: spec.aggregate })
    else { if (spec.values.length !== 1) throw new Error('透视表需要一个数值字段'); Object.assign(params, { rows: spec.rows, columns: spec.columns, values: [{ field: spec.values[0], aggregate: spec.aggregate }], name: '' }) }
  }
  ...
  if (operation.value === 'chart') {
    if (!keys.value.length) throw new Error('请选择一个分类字段（X 轴）')
    if (!columns.value.length) throw new Error('请选择至少一个数值字段')
    Object.assign(params, { chart_type: chartType.value, category: keys.value[0], values: columns.value, aggregate: 'sum', title: '' })
  }
...
<template>
  <div class="shell">
        ...
        <div v-if="view!=='result'" class="work-layout" :class="{'editor-layout':view==='editor'}">
          ...
          <section class="panel processing-panel"><h2>处理方式</h2><div v-if="view==='editor' && workbookContext" class="input-context"><strong>{{workbookContext.sheet_name}}</strong><p>当前选区 {{workbookContext.address}} · 版本 {{workbookContext.version}}</p><el-radio-group v-model="inputScope" :disabled="busy" @change="editorContext(workbookContext)"><el-radio-button value="sheet">整张表数据</el-radio-button><el-radio-button value="range">选区数据</el-radio-button></el-radio-group><p class="muted">数据输入首行为表头；编辑要求将显示选区并生成待核对候选。</p></div><el-tabs v-model="mode"><el-tab-pane label="常用操作" name="tools"/><el-tab-pane label="用文字描述" name="agent"/></el-tabs>
            <template v-if="mode==='tools'"><label class="field-label">想做什么？</label><el-select v-model="operation" aria-label="常用操作" :disabled="busy" @change="resetOperation"><el-option v-for="(label,key) in operations" :key="key" :value="key" :label="label"/></el-select>
              ...
              <p v-if="!newRule" class="operation-note">{{operation==='pivot'?'按行字段分组、列字段转成列名，对数值字段聚合，生成宽表矩阵。':operation==='pivot_table'?'生成原生 Excel 透视表（用 Excel 打开可继续调整字段）；数值字段须为数字。':view==='editor' && !twoFiles?'使用当前工作簿所选输入范围。':operation==='append'?'合并全部已导入文件；可分别点击文件调整工作表与表头。':operation==='join'?'保留主表全部记录，按同名字段匹配；补充表的关联字段必须唯一。':operation==='template'?'明确选择数据与模板，写入模板副本。':operation==='report'?'分析当前文件的字段类型、空值与重复，生成数据质量报告（不修改源文件）。':operation==='chart'?'按分类字段分组汇总数值字段，生成图表（前端预览 + Excel 原生图表）。':twoFiles?'按同名关键字段对比两份表格。':'使用当前选中的文件与工作表。'}}</p>
              <template v-if="usesKeys"><label class="field-label">{{['join','compare'].includes(operation)?'关联依据（同名字段）':operation==='chart'?'分类字段（X 轴，选 1 个）':'分组 / 保留字段'}}</label><el-select v-model="keys" multiple aria-label="关键字段" :disabled="busy" placeholder="从预览字段中选择"><el-option v-for="c in fields" :key="c" :value="c"/></el-select></template>
              <template v-if="usesColumns"><label class="field-label">{{operation==='group'?'求和字段':operation==='compare'?'对比字段（留空比较共同字段）':operation==='chart'?'数值字段':'处理字段'}}</label><el-select v-model="columns" multiple aria-label="处理字段" :disabled="busy"><el-option v-for="c in fields" :key="c" :value="c"/></el-select></template>
              <template v-if="operation==='chart'"><label class="field-label">图表类型</label><el-select v-model="chartType" aria-label="图表类型" :disabled="busy"><el-option v-for="t in chartTypes" :key="t.value" :value="t.value" :label="t.label"/></el-select></template>
              <PivotBuilder v-if="isPivot" :key="operation" ref="pivotBuilder" :task-id="task.id" :selection="selectedInput(activeFile)" :fields="fields.filter(c=>!c.startsWith('__source_'))" :busy="busy" :single-value="operation==='pivot_table'" />
              ...
              <el-button type="primary" class="wide execute" :disabled="busy || (view==='editor' && workbookContext?.readonly) || (!task.files.length && operation!=='create_table' && view!=='editor') || (isPivot && !activeFile) || (operation==='recalculate' && (!runtime.excel_ready || view==='editor'))" @click="action(execute)">{{task.status==='failed'?'按修改后的规则重试':'开始处理'}}</el-button><p v-if="operation==='recalculate' && !runtime.excel_ready" class="muted">Excel 原生重算不可用，请在设置中检测。</p>
            </template>
            <template v-else><p class="muted">例如：按物料汇总库存，再关联订单，列出未匹配记录。</p><el-alert v-if="!runtime.ready" title="请先在设置中配置智能处理；常用操作仍可使用。" type="info" :closable="false"/><div class="conversation"><div class="section-title"><el-button v-if="moreEvents" text @click="action(older)">更早的消息</el-button><el-button v-if="viewingOlder" text @click="action(latest)">返回最新消息</el-button></div><div v-for="event in displayedEvents" :key="event.id" class="message" :class="event.kind"><template v-if="event.kind==='step'"><strong>{{event.data.label || event.data.kind}}</strong><span v-if="stepRows(event.data)" class="step-rows">{{stepRows(event.data)}}</span><span v-if="event.data.issues" class="step-issues">{{event.data.issues}} 条异常</span></template><template v-else>{{message(event.data)}}</template></div><div v-if="busy && liveStreamText" class="message assistant streaming">{{liveStreamText}}<span class="stream-caret"/></div></div>
              ...
              <div class="scenario-chips"><button :disabled="busy" @click="prompt='新建一张采购填报表，表头为编号、物料、数量、单价、备注，留出20行空白。'">新建填报表</button><button :disabled="busy" @click="prompt='新增金额列，按数量乘以单价计算，保留两位小数。'">计算金额</button><button :disabled="busy" @click="prompt='把当前选区的表头设为粗体、蓝色背景，其他内容保持原样。'">设置表头格式</button><button :disabled="busy" @click="prompt='分析每个字段的类型、空值和重复情况，生成一份数据质量报告。'">数据质量报告</button><button :disabled="busy" @click="prompt='按物料汇总数量，再与订单关联，列出未匹配记录。'">多表关联汇总</button><button :disabled="busy" @click="prompt='解释当前选中单元格的公式在计算什么。'">解释公式</button></div><el-input v-model="prompt" type="textarea" :rows="4" aria-label="处理要求" placeholder="描述你想得到的结果…" :disabled="busy"/><el-button type="primary" class="wide execute" :disabled="busy || (view==='editor' && workbookContext?.readonly) || !runtime.ready || !prompt.trim()" @click="action(ask)">发送处理要求</el-button>
            </template>
    ...
    <AutomationPanel v-if="automationLoaded" v-model="automationOpen" :busy="busy" />
  </div>
</template>

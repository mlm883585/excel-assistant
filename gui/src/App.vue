...
onMounted(() => action(async()=> { await refreshLists(); settings.value=await call('settings.get'); if(history.value[0]) await selectTask(history.value[0]); else await newTask(); timer=setInterval(()=>action(poll),1200) }))
  ...
  <div class="shell">
    ...
    <main v-if="task"><header><div><div class="eyebrow">从数据到可交付的结果</div><h1>今天，需要整理什么数据？</h1></div><el-tag>{{labels[task.status]}}</el-tag></header>
      ...
      <div class="workspace"><section class="panel"><h3>2 · 描述要求</h3><p class="muted">例如：将库存按物料汇总，再关联订单，列出没有匹配的记录。</p>
        ...
        <div class="actions"><el-button type="primary" :disabled="busy || !task.files.length || !prompt.trim()" @click="action(askAgent)">开始处理 / 继续对话</el-button><el-button v-if="busy" @click="action(()=>call('tasks.cancel',{task_id:task!.id}))">取消任务</el-button></div>
        ...
        <p class="muted">关联使用左连接，右表关键字段须唯一；单表操作使用当前选中文件。模板按导入顺序选取数据、模板。</p><el-button type="primary" :disabled="busy || !task.files.length" @click="action(run)">按以上规则执行</el-button>
      </section></div>

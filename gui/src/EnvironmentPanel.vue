...
const updateSettings = ref({ base_url: '', enabled: true }), currentVersion = ref(''), updateResult = ref<{ update_available: boolean; latest_version?: string; url?: string; notes?: string; published_at?: string; reason?: string; error?: string }>({ update_available: false })
...
async function loadUpdate() {
  const got = await call('update.get')
  currentVersion.value = got.version || ''
  updateSettings.value = { base_url: got.base_url || '', enabled: got.enabled !== false }
}
async function checkUpdate() {
  updateResult.value = await call('update.check')
}
async function copyUpdateUrl() {
  const url = updateResult.value.url
  if (!url) return
  try { await navigator.clipboard.writeText(url); ElMessage.success('已复制下载地址') }
  catch { ElMessage.info('请手动选择并复制下方地址') }
}
...
<template>
  <el-drawer :model-value="modelValue" title="设置与环境" size="min(1050px, 96vw)" @update:model-value="emit('update:modelValue', $event)">
    ...
    <el-tabs v-model="tab"><el-tab-pane label="使用状态" name="overview"/><el-tab-pane label="模型连接" name="model"/><el-tab-pane label="软件更新" name="update"/><el-tab-pane label="高级诊断" name="advanced"/></el-tabs>
    ...
    <div v-if="tab==='update'" class="model-settings"><p class="muted">当前版本 <strong>{{currentVersion || '未知'}}</strong>。配置内网更新服务地址后可检查新版本并获取下载地址；更新为人工下载替换整个便携包，不自动改动本机。</p><label class="field-label">更新服务地址</label><el-input v-model="updateSettings.base_url" aria-label="更新服务地址" placeholder="http://内网地址:端口"/><p class="muted">留空表示未启用；也可由 IT 通过环境变量 EXCEL_ASSISTANT_UPDATE_URL 或打包默认值统一配置。</p><div class="environment-actions"><el-button type="primary" :disabled="pending || busy" @click="perform(async()=>{ await call('update.save', updateSettings); message='更新服务设置已保存'; messageType='success' })">保存更新设置</el-button><el-button :disabled="pending || busy" @click="perform(checkUpdate)">检查更新</el-button></div><el-alert v-if="updateResult.update_available" type="warning" :closable="false" show-icon title="发现新版本"><p>最新版本 <strong>v{{updateResult.latest_version}}</strong><template v-if="updateResult.published_at"> · {{updateResult.published_at}}</template></p><p v-if="updateResult.notes" class="muted" style="white-space:pre-wrap">{{updateResult.notes}}</p><el-input v-if="updateResult.url" :model-value="updateResult.url" readonly/><el-button v-if="updateResult.url" size="small" @click="copyUpdateUrl">复制下载地址</el-button></el-alert><el-alert v-else-if="updateResult.reason" type="info" :closable="false" show-icon :title="updateResult.reason"/><el-alert v-else-if="updateResult.error" type="error" :closable="false" show-icon :title="updateResult.error"/></div>
    ...
    <el-table v-if="tab!=='model' && tab!=='update'" :data="checks" stripe>
...
.runtime-candidate { border:1px solid var(--border); border-radius:8px; padding:12px; margin:10px 0; }
.runtime-path { overflow-wrap:anywhere; color:var(--text-muted); font-size:12px; margin:8px 0; }
</style>

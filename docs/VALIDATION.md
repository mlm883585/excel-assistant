...
更新日期：2026-09-15。Windows 11、Python 3.13.5、Node 24.13.0、Qwen Code 0.23.3、Univer 0.25.1。本次完成自有品牌替换，重跑完整 unittest、工作台与结果核对 UI、源码和便携包启动验证。完整公式 UI、20 万格性能及生命周期深度测量保留 2026-09-14 的基线，不记为本次重测；旧记录中的 41、69、71 项测试属于更早阶段。

## 品牌替换验收（2026-09-15）

| 检查 | 本次结果 |
| --- | --- |
| 原创资源 | SVG 母版、1024×1024 透明 PNG、16/24/32/48/64/128/256 像素 ICO；小尺寸目视检查通过 |
| 产品界面 | 工作台标识、favicon、中文窗口标题、EXE 与原生窗口图标统一；应用标识与数据目录不变 |
| PPX 素材清理 | 应用及源码中 8 份旧图片删除；宣传图片、赞助和公众号内容移除；初始化不会重新下载旧图片 |
| 必要归属 | 保留上游作者、技术名称、源码及许可证；PPX Python 许可证归档到 `licenses/ppx/LICENSE` |
| 实际图标 | EXE 的 7 份内嵌图标数据与 ICO 一致；源码启动及便携包均比较了实际原生窗口图标像素，并验证窗口显示于任务栏 |
| WebView2 | 源码和打包后实际 edgechromium 启动通过；标题“Excel 数据助手”，品牌图已加载，资源记录外网请求为 0 |
| 工作台与核对 | 千条任务、万条消息、宽表、十万行翻页、整份采用、继续编辑、导出取消及过期确认均通过；3 张合成数据截图更新 |

详细维护方法见 [品牌资源说明](BRANDING.md)。

## 功能与协议

| 检查 | 本次结果 |
| --- | --- |
| 完整 unittest | 74 项通过（93.858 秒）；新增初始化排除旧图、模板缺失时无写入和窗口参数兼容回归 |
...
| 桌面 `--smoke` | 2026-09-15：实际 `ExcelAssistant.exe` 启动 WebView2，验证标题、品牌图和原生窗口图标后正常退出，退出码 0 |
| 打包后 MCP 与固定 CLI | 2026-09-15：实际 `DataCraftMCP.exe` 通过真实 stdio 合并与导出测试；随包 Qwen Code 0.23.3 与固定 Node 协议探测通过；新增操作覆盖在完整 unittest 中 |
| 打包后环境诊断 | 依赖探测与 Agent 探测均通过，覆盖实际 CLI 会话、产物、权限拒绝及取消 |
| 打包后十万行预览 | 实际桌面可执行文件生成后台索引；2026-09-15：首批 0.501 秒、缓存分页后端 P95 0.640 毫秒，编号及 NA 文本保持 |
| 对应源码、前端、许可证与哈希 | `scripts/verify_release.py` 通过：3443 个交付文件、379 个源码文件、184 个运行依赖许可证、88 个前端资源；无遗漏或遗留前端文件，总体积约 669.7 MiB；旧图残留为 0，EXE 内 7 种尺寸图标一致 |

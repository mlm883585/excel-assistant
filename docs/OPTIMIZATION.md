...
- **方案**：把 SQLite + 任务目录抽成可服务化存储，`api/api.py` 加鉴权与租户。**远期**，不在短期路线。分阶段设计已出，见 [`docs/COLLABORATION.md`](COLLABORATION.md)（阶段一存储可服务化 → 阶段二 HTTP 服务层 → 阶段三鉴权/租户 → 阶段四共享数据源；触发条件为企业需求 + 内网安全评审）。
...
| **Phase D（远期）** | 方向 9 沙箱执行（排除：不执行代码约束）+ 方向 11 协作服务化（暂缓）+ 方向 12 性能（已完成 2026-09-19） | 方向 12 已落地；方向 9/11 视安全评审与企业需求推进 |
